...
**Sesión de hoy (01/09) — todo pusheado a `main`, `tsc --noEmit` limpio en cada paso:**

1. **Subir captura + lectura por IA en Cobros/Subcontratos/Trabajo puntual (Reporte.tsx).** Mismo mecanismo que Compras (sube a `audio-notas`, lee con `/api/parse-comprobante`, completa el Monto solo). Migración `sql/20260901_comprobantes_cobros_subcontratos_trabajos.sql` — **ya corrida por Alexandra, confirmado vía MCP**. No se pudo probar la subida real de un archivo ni la lectura por IA desde acá (límite de siempre: Cloudflare Functions no corren en Vite local); queda para la primera captura real en producción.

2. **Borrar ajuste/adelanto en Pago Semanal**, **borrar fecha mal seleccionada en Obras**, **botón Borrar en la ficha de cliente**, **botón "+ Agregar cliente"** (antes no existía ninguna forma de crear un cliente nuevo — `clientes` solo se llenaba con un backfill único desde `pendientes`; ahora crea con el nombre y salta directo a cargar su facturación). Todo verificado en navegador contra datos reales.

3. **Chat "Consultas IA" para Gustavo** (pestaña nueva, solo panel Gustavo) — responde en base a un resumen de datos reales (obras/pago semanal/cuentas) armado por el frontend, sin acceso directo a la base. Se extrajo `calcularResumenObras()` compartida para que el saldo que ve el chat sea siempre igual al de la pestaña Obras. Función `functions/api/chat-ia.js` sin poder probarse contra la API real desde acá (mismo límite de Cloudflare Functions) — **pendiente que Alexandra confirme que responde bien una vez usada en producción**.

4. **Fusión de "Trabajadores" + "Historial de pagos" en una sola pestaña**, a pedido de Alexandra (el mismo trabajador se buscaba en dos lugares separados). Click en un trabajador abre su ficha con Archivar/Obra asignada + historial. Para tarifa diaria se agregó el segmentador **Semana/Quincena/Mes** que faltaba — **verificado contra Supabase real** (Henry: Agosto $1.110.000 Neto y Septiembre $60.000 coinciden exacto con la suma directa en `reportes_diarios`). Para sueldo fijo (Fabriel) se mantuvo la vista mensual y se agregó: botón de borrar un adelanto mal fechado, comprobante del pago mensual completo (antes solo existía para los pedacitos de viático semanal), aviso "✓ Pago del mes confirmado" que relabela "Resta pagar"→"Pagado" cuando el comprobante coincide (para no contradecirse), y que el mes calendario actual siempre aparezca aunque todavía no tenga nada cargado (si no, no había manera de "entrar" a un mes nuevo para cargarle un adelanto). **Caso real ya resuelto por Alexandra usando estas herramientas:** el adelanto de $100.000 de Fabriel (14/08) se movió de agosto a septiembre a pedido del propio Fabriel — borrado y recargado con fecha de septiembre, confirmado con comprobante subido y coincidencia verificada.

5. **Bug sistémico de contraste encontrado y corregido en 4 lugares** (reportado por Alexandra con capturas): varios contenedores de fondo claro (`var(--white)`/`var(--surface)`) no definían su propio `color`, así que heredaban el texto claro que la app usa por defecto para el fondo navy — invisible sobre fondo blanco. Corregido en la raíz de cada contenedor (no parche por parche): modal "Detalle" de obra, modal de detalle de presupuesto, modal de historial de cliente en Admin, y el formulario "Nuevo pendiente"/"Editar pendiente" en Admin (esto último explica un problema real: Alexandra cargó **2 pendientes duplicados de "emitir_factura" para Patricia Marambio**, misma factura de Mga Abogados Ltda por $125.446, porque no veía bien lo que tipeaba — **sin borrar todavía, a la espera de que confirme cuál de los dos IDs eliminar**, ver `decisiones.md`).

6. **Archivado de pendientes viejos en la pestaña "Gustavo" (Admin).** Esa vista mezclaba un cliente real y activo (Patricia Marambio, "Por revisar") con ~35 clientes de hace meses ya marcados "Listo", sin forma de sacarlos de la vista. Migración `sql/20260901_pendientes_archivado.sql` (columna `archivado` en `pendientes`) — **ya corrida por Alexandra**. Botón "Archivar" por cliente + botón masivo **"Archivar todos los 'Listo'"** (archiva de una vez a todos los ya revisados, sin tocar los pendientes de revisar) — **construido y pusheado, pero sin poder probarse en vivo** (límite de siempre: Admin.tsx no corre en Vite local por el login vía Cloudflare Function) y **sin ejecutar todavía** — queda para que Alexandra lo use la próxima vez que entre a Admin en producción.

**Pendiente para la próxima sesión:**
- Confirmar y borrar uno de los 2 pendientes duplicados de Patricia Marambio (`emitir_factura`) — IDs `2fa1c38f...` (14:42, sin monto) y `c90334af...` (21:03, con "Monto a facturar: $125.446"). Recomendado borrar el primero.
- Borrar el cobro real de $700 del 29/08/2026 en "Camino turístico 11474, Lo Barnechea" (`reportes_cobros` id `456af00b...`) — es un error de tipeo de Gustavo (faltaban tres ceros), ya corregido con una fila nueva de $700.000 el 01/09, pero la fila mala nunca se borró. Sin confirmar/ejecutar todavía.
- Confirmar en producción que "Consultas IA" responde bien (chat-ia.js sin poder probarse desde acá).
- Probar en producción "Archivar todos los 'Listo'" en Admin → Gustavo (Admin.tsx no verificable en vivo desde acá).
- Confirmar que la captura+IA de Cobros/Subcontratos/Trabajo puntual lee bien un monto real (sin poder simular la subida de un archivo desde acá).

## Última actualización anterior: 28/08/2026 (cierre de sesión larga — bitácora de avance Fase 1, Reporte Diario con confirmación de guardado + compras colapsadas, Banco de contenido con visor, fixes de obra-fotos)

## Para la próxima sesión (lunes 31/08) — empezar acá

**Estado final de Reporte Diario, para no repetir vueltas ya hechas hoy** (se iteró varias veces en la misma sesión, esto es lo que quedó y por qué):
- El guardado en sí **no cambió**: `reportes_diarios` (asistencia) usa `upsert` por fecha+trabajador; compras/cobros/subcontratos/trabajos puntuales/uso de stock siguen usando "borrar todo lo de esa fecha y reinsertar lo que hay en el formulario" — mecanismo viejo, ya probado, no tocar sin razón de peso.
- **Se probó y SE REVIRTIÓ dejar el formulario completamente en blanco tras guardar** (lo pidió Alexandra en un primer momento): con el guardado "borra todo y reinserta", un formulario vacío en un segundo guardado el mismo día borraba lo guardado en el primero — bug real, encontrado antes de causar daño. **No reintentar el "formulario en blanco" sin antes cambiar ese guardado a upsert real por fila** (insertar solo lo nuevo, actualizar solo lo existente, borrar solo lo que se saca a mano con "Quitar") — si no, vuelve el mismo bug.
- **Lo que sí quedó, y resuelve el pedido real de Alexandra sin ese riesgo:** al guardar, la pantalla sube sola arriba de todo y muestra un cartel verde grande e imposible de no ver ("✓ Reporte guardado correctamente"), 7 segundos. Además, cada compra que ya tiene `id` (ya guardada) se ve como una tarjeta chica y cerrada ("✓ Cortadora de cerámica — $179.990" + botón "Editar") en vez del formulario completo — se colapsan solas al cargar el día. "+ Agregar compra" siempre abre una nueva en blanco sin tocar las demás. Mismo patrón se podría aplicar a Cobros/Subcontratos/Trabajos puntuales si Alexandra lo pide — hoy solo Compras lo tiene.
- Se agregó también un aviso de "posible compra duplicada" (mismo monto, mismo día) antes de guardar, igual que ya existía para cobros.
- Todo esto verificado con Playwright **interceptando toda escritura de red** contra Supabase real (nunca dejar pasar un guardado real al probar este formulario) y confirmado en producción con el bundle desplegado.

**⚠️ Incidente real de hoy (28/08), ya resuelto — dejarlo documentado para no repetirlo:** probando el aviso de compra duplicada, un test de Claude sí disparó un guardado REAL de asistencia (marcó a los 6 trabajadores activos "Ausente" hoy) porque el guardado de asistencia corre ANTES en el código que la validación de compras que se quería probar — canceló el diálogo pensando que eso alcanzaba para no guardar nada, pero ya era tarde. Alexandra confirmó con Gustavo los datos reales y él lo corrigió a mano; se verificó contra Supabase que quedó bien y sin duplicados (ni Gustavo ni Alexandra guardando por separado duplicaron nada — `reportes_diarios` usa upsert, no inserta filas nuevas). **Regla dura para el futuro: cualquier prueba con Playwright que vaya a tocar un botón "Guardar" real tiene que interceptar TODA escritura de red de antemano, sin excepción — nunca asumir que una parte del flujo es "de todos modos de solo lectura".**

**Pendiente para el lunes:**
- **Alejandro ya no trabaja con Horma** (confirmado por Gustavo) — se ofreció archivarlo desde la card de Trabajadores (botón ya existe) para que deje de aparecer en la asistencia diaria; quedó sin confirmar si Alexandra quiere que lo haga la próxima sesión o si ya lo hizo ella misma.
- Descarga masiva + borrado de fotos por obra cerrada — anotado en `tareas.md`, bloqueado a propósito (bucket sin política `DELETE` para `anon`), necesita decisión de Alexandra sobre el modelo de acceso antes de construirse.
- Fase 2 (costo en plata del atraso) y Fase 3 (reglas de bono) de la bitácora de avance — 4 preguntas de negocio sin resolver, ver `decisiones.md` 2026-08-28.
- Plan de Supabase: confirmado Free, sin backups. Deciden pasar a Pro el 2 de septiembre de 2026 (cuando les paguen) — no es urgente adelantarlo, pero extremar cuidado con operaciones destructivas hasta esa fecha.
- `sql/20260828_trabajadores_obra_asignada.sql` — confirmar si ya se corrió (quedó pendiente de una sesión anterior).

**Todo lo de esta sesión ya está pusheado a `main`**, `tsc --noEmit` limpio en cada paso. Commits del día (orden cronológico): `c93036c` (bitácora Fase 1), `558a7ba` (fixes obra-fotos + visor banco de contenido), `6219587` (Reporte Diario — primer intento, ya superado), `3681e76` (revert del formulario en blanco + aviso duplicado), `13f1c76` (scroll-to-top + cartel grande), `565e3f2` (compras colapsadas, versión final).
...
0. **PRIORIDAD MÁXIMA — conversación de seguridad de fondo, sin resolver, iniciada el 28/08 (y todavía sin tocar después de toda la sesión de rediseño).** Alexandra preguntó directamente si lo que le dijo a Gustavo ("con Supabase estamos seguros") es cierto, porque no quiere engañar a quien confía en ella. Se le contestó con honestidad, verificando en vivo (no de memoria):
   ...
   - **RESUELTO y CONFIRMADO 31/08:** Supabase pasó a plan Pro (adelantado del 2/09 planeado) — backups diarios automáticos ya funcionando, confirmado viendo Database → Backups (backups desde el 26/08, el más reciente del mismo 31/08). **Ojo:** esos backups NO incluyen Storage (fotos, comprobantes, boletas del bucket `audio-notas`) — solo tablas de la base de datos. Ver `progress/decisiones.md` 2026-08-31.
