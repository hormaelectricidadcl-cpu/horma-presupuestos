...
| 2.5 | **Galería de fotos/videos por obra en la web app** (propuesta de Alexandra, 25/08) — reemplaza tener que abrir Google Drive para ver el material de una obra. | **Código listo, bloqueado en Alexandra corriendo `sql/20260825_obra_media.sql`** — Toda la "vida de la obra" (fotos de avance, videos) queda dentro de la app, sin depender de que alguien encuentre la carpeta correcta en Drive |
...
- `tsc --noEmit` limpio, `init.sh` en verde. Probado en navegador (Gustavo → Obras → Detalle de "Luis Carrera 2700") antes de correr la migración: la sección "Fotos y videos" carga arriba de todo, sin errores que rompan la pantalla, muestra "Sin fotos ni videos todavía" (falla silenciosa porque la tabla no existe aún). **Falta correr la migración y volver a probar subiendo un archivo real.**
...
| 2.9 | **`referencia` (ID) en presupuestos por etapas** | El presupuesto simple ya genera un ID `HRM-XXXXX` (desde antes de esta sesión) y lo guarda (1.1); el presupuesto por etapas (`PresupuestoEtapas.tsx`) nunca lo tuvo — sus filas en "Mis presupuestos" no muestran referencia. Agregar la misma generación ahí para que todo presupuesto tenga un ID real, sin excepción. |
...
**Todavía sin construir — pendiente que Alexandra confirme el orden.** No se hizo nada de esto todavía, se dejó documentado para no perder ningún hilo de la conversación.
